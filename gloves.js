...
async function begin() {
  ...
  posX = width / 2;
  posY = height / 2;
  await game.mouse.moveTo(posX, posY);
...
async function arduinoIn(value) {
  ...
  last_data = value;
  //console.log("different msg: " + value);
  same_msg = false;
...
async function processLook() {
  if (t === 2 && !m && !r) {
    ...
    if (Math.abs(diff) > 2) {
      posX += diff * 4;
      await game.mouse.humanMoveTo(posX, posY, 5, 0);
      start_x = x;
    }

    diff = y - start_y2;
    if (Math.abs(diff) > 2 && !i) {
      //console.log('up ' + y + ":" + start_y2);
